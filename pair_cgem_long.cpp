...
PairCGemLong::PairCGemLong(LAMMPS *lmp) : Pair(lmp)
{
...
void PairCGemLong::compute(int eflag, int vflag)
{
  ...
  for (int ii = 0; ii < inum; ii++) {
    ...
    for (int jj = 0; jj < jnum; jj++) {
      ...
      if (dr2 < cutsq[itype][jtype]) {
        ...
        const double ferfg=std::erf(rmuC[itype][jtype]*dr);
        ...
        if(dr>1e-8){
          fCoul=prefactor/dr2*(
            (ferfg-ferfp)
            +2.0/MY_PIS*dr*(
              -rmuC[itype][jtype]*std::exp(-muC[itype][jtype]*dr2)
        ...
        if (eflag) {
          ...
          if(dr>1e-8){
            ...
            ecoul = 2.0/MY_PIS*qqrd2e*qi*qj*(rmuC[itype][jtype]-g_ewald);
          }
        }
...
void PairCGemLong::settings(int narg, char **arg)
{
  if (narg != 3) error->all(FLERR, "Illegal pair_style command");
  cut_global = utils::numeric(FLERR, arg[0], false, lmp);
  lambdaC = utils::numeric(FLERR, arg[2], false, lmp);
  lambdaS = utils::numeric(FLERR, arg[1], false, lmp);
...
void PairCGemLong::coeff(int narg, char **arg)
{
...
double PairCGemLong::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairCGemLong::write_restart(FILE *fp)
{
...
void PairCGemLong::read_restart(FILE *fp)
{
...
void PairCGemLong::write_restart_settings(FILE *fp)
{
...
void PairCGemLong::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
void PairCGemLong::write_data(FILE *fp)
{
...
void PairCGemLong::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++){
    for (int j = i; j <= atom->ntypes; j++){
...
double PairCGemLong::single(int i, int j, int itype, int jtype, double rsq,
                         double factor_coul, double factor_lj, double &fforce)
{
  ...
  const double ferfcg=std::erfc(rmuC[itype][jtype]*r);
  const double ferfcp=std::erfc(g_ewald*r);
  double eCoul = factor_coul*prefactor*(-ferfcg+ferfcp);
  double fCoul=prefactor/rsq*(
    (-ferfcg+ferfcp)
    -2.0/MY_PIS*r*(
      -rmuC[itype][jtype]*std::exp(-muC[itype][jtype]*rsq)
  ...
  // overlap
  const double eOver=aOver[itype][jtype]*std::fabs(q[i])*std::fabs(q[j])*std::exp(-muS[itype][jtype]*rsq)*factor_lj;
  double fOver=2.0*muS[itype][jtype]*eOver;
...
void *PairCGemLong::extract(const char *str, int &dim)
{
