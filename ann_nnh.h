...
/*
PRIVATE:
	int ntypes_ - the total number of species in a neural network potential
	Type type_ - the central atom of the NNH
	NN::ANN nn_ - the neural network which determines the energy of the central atom
	basisR_ - the radial basis functions
		there is a radial basis for each species in the simulation (ntypes)
	...
	basisA_ - the angular basis functions
		there is an angular basis for each unique pair of species in the simulation (ntypes x (ntypes-1)/2)
...
private:
	...
	//hamiltonian
	int ntypes_;//the total number of species
	Type type_;//atom - name, mass, energy, charge
...
public:
	...
	//hamiltonian
		...
		NN::DODZ& dOutDVal(){return dOdZ_;}
		const NN::DODZ& dOutDVal()const{return dOdZ_;}
	//basis for pair/triple interactions
